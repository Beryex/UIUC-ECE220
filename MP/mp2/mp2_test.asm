...
; R6 is used to holds the target address
		...
		LEA R1,List_addr	; let R1 holds the address x5000  need to change to LD R1,List_addr later
...
Days_loop	LD R0,Vertical_line	; let R0 stores x7C to print a vertical line
		OUT		; print a vertical line
		ADD R1,R5,#0	; let R1 stores the address of the string
		JSR PRINT_CENTERED	; call PRINT_CENTERED to print the string
...
List_addr		.STRINGZ "1234567"		; name of event
		.FILL x0015		; bitmap of days (MWF)
		.FILL x0006		; slot 6, or 13:00
		.STRINGZ "123455"		; name of event
