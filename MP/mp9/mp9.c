...
/*
    ...
    in the range of locale_t and store them into vertex_set_t. trim_nodes can 
    remove any vertex that is not in the range of locale_t from vertex_set_t.
    dijkstra can find a shortest path that starts in src and ends in dest, find
    the minimum distance and store the path vertex into path->id[]. dijkstra uses
    a heap to implement dijkstra algorithm, it uses several functions to implement
    operation to the heap.
*/
...
find_nodes (locale_t* loc, vertex_set_t* vs, pyr_tree_t* p, int32_t nnum)
{
    ...
    if (vs -> count >= MAX_IN_VERTEX_SET){                  // if having identify MAX_IN_VERTEX_SET vertices, stop
        return;
    }
